...
$pdf->Cell(0,4,'Payment Methods',0,1);
$pdf->Ln(1);
$pdf->SetFont('freesans','',7);
$paymentHTML  = '<P>Please note this invoice needs to be paid online by going to https://clients.prolateral.com.<BR>';
$paymentHTML .= 'Payment methods accepted are: Credit/Debit card or PayPal.</P>';
...
if (!$taxrate) {
   $pdf->Ln(5);
   $pdf->SetFont('freesans','B',9);
   $pdf->Cell(0,4,'Notice on EU VAT',0,1);
   $pdf->Ln(1);
   $pdf->SetFont('freesans','',7);
   $paymentHTML  = '<P>If you are VAT registered and you have provided your VAT number in the client details area, then you will not see VAT above and no VAT has been charged.</P>';
   $pdf->writeHTML($paymentHTML, true, false, false, false, '');
}
...
$pdf->Ln(5);
$pdf->SetFont('freesans','B',9);
$pdf->Cell(0,4,"Let's get \"Social\" together",0,1);
$pdf->Ln(1);
$pdf->SetFont('freesans','',7);
$paymentHTML  = '<P>Please like us on FaceBook (www.facebook.com/prolateral) and watch out for our promotional offers.<BR>';
$paymentHTML .= 'Why not follow @prolateral on Twitter as well? (www.twitter.com/prolateral).</P>';
$pdf->writeHTML($paymentHTML, true, false, false, false, '');
